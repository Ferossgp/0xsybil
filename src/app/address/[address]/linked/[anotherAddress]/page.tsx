...
const RenderTxs: React.FC<{ txs: LinkedTransactions[] }> = ({ txs }) => {
  return (
...
export default function Page(params: Params) {
  ...
  const { data, isLoading, } = useSWR(`/api/linkedTransactions?address=${address}&anotherAddress=${anotherAddress}`, fetcher);
  ...
  const txs = (data?.transactions ?? []) as LinkedTransactions[]
  ...
  return (
    <div className="flex flex-col flex-1 gap-8">
      ...
      <div className="flex flex-col gap-6 items-center flex-1">
        <h1 className="text-2xl">Linked transactions<span className="font-mono">
          </span></h1>
